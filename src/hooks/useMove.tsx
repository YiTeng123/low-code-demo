...
import {
  ...
  updateSelectedComponentWrapperStyle,
} from '../store/helper.ts'
...
const createYLineMap = (
  targetStyle: CSSProperties,
  selectedStyle: CSSProperties
) => {
  const distanceLR =
    (targetStyle.left as number) +
    (targetStyle.width as number) -
    (selectedStyle.left as number)
  const distanceLC =
    (targetStyle.left as number) +
    (targetStyle.width as number) / 2 -
    (selectedStyle.left as number)
  const distanceRR =
    (targetStyle.left as number) +
    (targetStyle.width as number) -
    ((selectedStyle.left as number) + (selectedStyle.width as number))
  const distanceRC =
    (targetStyle.left as number) +
    (targetStyle.width as number) / 2 -
    ((selectedStyle.left as number) + (selectedStyle.width as number))
  const distanceLL =
    (targetStyle.left as number) - (selectedStyle.left as number)
  const distanceRL =
    (targetStyle.left as number) -
    ((selectedStyle.left as number) + (selectedStyle.width as number))
  return {
...
const autoAlignWhenMove = (storeState: EditorStore) => {
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    if (c.wrapperStyle && selectedComponent.wrapperStyle) {
      const yMap = createYLineMap(
        c.wrapperStyle,
        selectedComponent.wrapperStyle
      )
      map(yMap, (v, k) => {
