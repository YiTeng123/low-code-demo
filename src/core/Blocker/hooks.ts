  ...
  const outerStyle = useMemo(() => {
    ...
    for (let i = 0; i < selectedComponents.length; i++) {
      ...
      if (current.wrapperStyle) {
        const { left, top, width, height } = current.wrapperStyle
        if (!height || !width) {
          return {
            left: 0,
            top: 0,
        ...
        maxLeftWithWidth = Math.max(
          maxLeftWithWidth,
          (left as number) + (width as number)
        )
        maxTopWithHeight = Math.max(
          maxTopWithHeight,
          (top as number) + (height as number)
        )
      }
    }
    return {
